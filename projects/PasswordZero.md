...
For instance, the workflow steps are listed below.
...
A couple of things to remember are that you want to ensure the SSH key you're using is configured to have read-only access to the repository. Additionally, you should include steps in your deployment to clean up the secrets and the git repository, especially since the job runs only once at instance instantiation.

#### Pros
* Very simple and easy to set up.
* Offers excellent flexibility.
* Allows image templating.
  
#### Cons
* Plain text passwords stored in the database.
* Delay in server readiness.
